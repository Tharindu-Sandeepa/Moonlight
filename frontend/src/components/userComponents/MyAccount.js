...
const MyAccount = () => {
    ...
    const fetchUserDetails = async () => {
        try {
            ...
            const response = await axios.get('http://localhost:5002/api/user/myaccount', config);
    ...
    // Fetch user details initially and whenever the user data changes
    useEffect(() => {
        fetchUserDetails();
    }, []);
    ...
    const updateUser = async (data) => {
        ...
        try {
            ...
            setShowForm(false);
            // Fetch user details again to reflect the changes
            fetchUserDetails();
        } catch (error) {
